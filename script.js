...
$("ul#books").children().click(() => {
	$("ul#books").children().remove()

	for (i = 0; i < j; i++) {
		$("ul#books").append(
	`<li class="grid__item grid__item--4"><a class="grid__link"><h2 class="grid__title">AFORISMO ${i + 1}</h2><p>Meditaciones</p></a></li>`)
	}
	for (i = 0; i < j; i++) {
$('.article').append(
	`<section class="post">
   ...
   <span id="voice" class="material-icons">mic_none</span>
   <span id="share" class="material-icons">share</span>
   </div>
	</article>
	...
	ZoomGrid.init();
	$("#book_icon").click(function () { location.reload() })
