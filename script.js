...
text=fetch('Meditacones.json').then(Response =>Response.json()).then(data=>{text= data})
...
$('.article').append(`<section class="post">
	...
	<h1>LIBRO ${k+1}</h1>
	<h3>Aforismo ${i+1}</h3>
	<p class="text">${text[k][i]}</p>
	<div class="icons_container">
   <button id="btnVoice"><span class="material-icons">mic_none</span></button>
   <button id="btnShare"><span class="material-icons">share</span></button>
   </div>
	</article>
...
$('#btnVoice').click(function(){
	var uterance = new SpeechSynthesisUtterance($('.text').text());
	speechSynthesis.speak(uterance);
})
$('#btnShare').click(function(){
	navigator.share({
		title:'Meditaciones de Marco Aurelio',
		text:'jaajajajajajajajajaajjajaaj',
		url:$('.text').text(),
	})
})
});